...
function Flipsnap(element, defaultPage, opts) {
	return (this instanceof Flipsnap)
		? this.init(element, defaultPage, opts)
		: new Flipsnap(element, defaultPage, opts);
}

Flipsnap.prototype.init = function(element, defaultPage, opts) {
	var self = this;
...
//	self._distance = self.distance || self.element.scrollWidth / (self._maxPoint + 1);
	...
	if (support.cssAnimation) {
		self._setStyle({
			...
			transform: self._getTranslate(-self._distance * self.currentPoint)
		});
	}
	else {
		self._setStyle({
			position: 'relative',
			left: (self._distance * self.currentPoint) + 'px'
		});
	}
